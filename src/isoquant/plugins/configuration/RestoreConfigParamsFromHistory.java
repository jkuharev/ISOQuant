...
/**
 * <h3>{@link RestoreConfigParamsFromHistory}</h3>
 * @author kuharev
 * @version 10.07.2013 09:29:52
 */
public class RestoreConfigParamsFromHistory extends SingleActionPlugin4DB
{
	public RestoreConfigParamsFromHistory(iMainApp app)
	{
		super(app);
	}
	...
	{
		Map<String, String> dbCfg = p.mysql.getMap("SELECT DISTINCT value, note FROM `history` WHERE type='parameter' ORDER BY id DESC", "value", "note");
		...
		{
			app.showErrorMessage("no ISOQuant configuration parameters were found in project history!");
			return;
		}
	...
	private Object[][] showParamList(Map<String, String[]> params)
	{
		...
		TableFactory tf = new TableFactory(td, new Object[] { "parameter", "current value", "history value" }, new Boolean[] { false, false, true });
		final JScrollPane tabPane = tf.getScrollableTable(true);
		XGUIUtils.makeParentDialogResizable(tabPane);
		class OP extends JOptionPane
		{}
		int ok = JOptionPane.showConfirmDialog(
				app.getGUI(),
				tabPane,
				"parameters to import from history",
	...
	{
		return "restore config from history";
	}
